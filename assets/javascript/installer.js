...
(function ($) {
  ...
  function setProgress(progressObject) {
    ...
    if (progressObject.task_name.length) {
      ...
      incrementProgressBar(
        Math.round((progressObject.task_num / progressObject.task_count) * 100)
      );
    }
  }
  ...
  function setCookies(cookies) {
    ...
    for (var i = 0; i < cookies.length; i++) {
      ...
      cookie =
        encodeURIComponent(cookies[i].name) +
        "=" +
        encodeURIComponent(cookies[i].value);
      // Set path
      cookie += "; path=/";
  ...
  function incrementProgressBar(progressLimit) {
    ...
    var progressStart =
      ($progressFiller.width() / $progressFiller.offsetParent().width()) * 100;
  ...
  function getFormFields($form, $submitBtn) {
    ...
    formData +=
      (formData.length ? "&" : "") +
      encodeURIComponent($submitBtn.attr("name")) +
      "=";
