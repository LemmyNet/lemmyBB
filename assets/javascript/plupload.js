...
(function ($) {
  ...
  phpbb.plupload.getSerializedData = function () {
    ...
    obj.creation_time = $pluploadForm
      .find('input[type=hidden][name="creation_time"]')
      .val();
    obj.form_token = $pluploadForm
      .find('input[type=hidden][name="form_token"]')
      .val();
  ...
  phpbb.plupload.updateRow = function (index, downloadUrl) {
    ...
    if (
      ...
      typeof downloadUrl[index] !== "undefined"
    ) {
  ...
  phpbb.plupload.handleMaxFilesReached = function () {
    ...
    if (phpbb.plupload.maxFiles <= phpbb.plupload.ids.length) {
      ...
      phpbb.plupload.uploader.trigger("Error", {
        message: phpbb.plupload.lang.TOO_MANY_ATTACHMENTS,
      });
  ...
  phpbb.plupload.uploader.bind("Error", function (up, error) {
    ...
    if (error.code === plupload.FILE_EXTENSION_ERROR) {
      error.message =
        plupload.translate("Invalid file extension:") + " " + error.file.name;
    } else if (error.code === plupload.FILE_SIZE_ERROR) {
      error.message =
        plupload.translate("File too large:") + " " + error.file.name;
    }
