...
(function ($) {
  ...
  phpbb.showTimeoutMessage = function () {
    ...
    if ($loadingIndicator.is(":visible")) {
      phpbb.alert(
        $alert.attr("data-l-err"),
        $alert.attr("data-l-timeout-processing-req")
      );
    }
  };
  ...
  phpbb.confirm = function (msg, callback, fadedark) {
    ...
    $confirmDiv
      ...
      .one("click.phpbb.confirmbox", function (e) {
  ...
  phpbb.ajaxify = function (options) {
    ...
    $elements.on(eventName, function (event) {
      ...
      if (
        $this.find('input[type="submit"][data-clicked]').attr("data-ajax") ===
        "false"
      ) {
        return;
      }
      ...
      function errorHandler(jqXHR, textStatus, errorThrown) {
        if (typeof console !== "undefined" && console.log) {
          console.log(
            "AJAX error. status: " + textStatus + ", message: " + errorThrown
          );
        }
        phpbb.clearLoadingTimeout();
        var responseText,
      ...
      var sendRequest = function () {
        ...
        if (
          overlay &&
          (typeof dataOverlay === "undefined" || dataOverlay === "true")
        ) {
          phpbb.loadingIndicator();
        }
    ...
    if (isForm) {
      $elements.find("input:submit").click(function () {
        ...
        $this
          .parents("form:first")
          .find("input:submit[data-clicked]")
  ...
  phpbb.search.setValueOnClick = function ($input, value, $row, $container) {
    $row.click(function () {
      phpbb.search.setValue(
        $input,
        value.result,
        $input.attr("data-multiline")
      );
  ...
  phpbb.search.filter = function (data, event, sendRequest) {
    var $this = $(this),
      dataName =
        $this.attr("data-name") !== undefined
          ? $this.attr("data-name")
          : $this.attr("name"),
      minLength = parseInt($this.attr("data-min-length"), 10),
      searchID = $this.attr("data-results"),
      keyword = phpbb.search.getKeyword(
        $this,
        data[dataName],
        $this.attr("data-multiline")
      ),
    ...
    var timeout = setTimeout(function () {
      ...
      if (minLength > keyword.length) {
        ...
        if (cache.lastSearch === keyword) {
          ...
          if (
            keyword.indexOf(cache.lastSearch) === 0 &&
            cache.results[cache.lastSearch].length === 0
          ) {
   ...
   */
  phpbb.search.navigateResults = function (
    $input,
    $container,
    $resultContainer
  ) {
  ...
  phpbb.history.isSupported = function (fn) {
    return !(
      typeof history === "undefined" || typeof history[fn] === "undefined"
    );
  };
  ...
  phpbb.timezoneSwitchDate = function (keepSelection) {
    ...
    if ($tzDate.val() !== "") {
      $timezone
        .children("optgroup")
        .remove(':not([data-tz-value="' + $tzDate.val() + '"])');
    }
    ...
    var $tzOptions = $timezone
      .children('optgroup[data-tz-value="' + $tzDate.val() + '"]')
      .children("option");
  ...
  phpbb.timezonePreselectSelect = function (forceSelector) {
    ...
    for (i = 0; i < selectorOptions.length; ++i) {
      ...
      if (option.value.substring(0, prefixLength) === prefix) {
        ...
        $tzSelectDateSuggest.attr(
          "title",
          suggestion.replace("%s", option.innerHTML)
        );
        $tzSelectDateSuggest.attr(
          "value",
          suggestion.replace("%s", option.innerHTML.substring(0, 9))
        );
  ...
  phpbb.addAjaxCallback("member_search", function (res) {
    phpbb.search.handleResponse(
      res,
      $(this),
      false,
      phpbb.getFunctionByName("phpbb.search.setValueOnClick")
    );
  });
...
*/
  phpbb.resizeTextArea = function ($items, options) {
    ...
    function autoResize(item) {
      ...
      var maxHeight = Math.min(
          Math.max(
            windowHeight - configuration.heightDiff,
   ...
   */
  phpbb.dropdownHandles =
    ".dropdown-container.dropdown-visible .dropdown-toggle";
  ...
  phpbb.toggleDropdown = function () {
    ...
    if (!visible) {
      ...
      parent
        .toggleClass(options.leftClass, direction === "left")
      ...
      parent
        .toggleClass(options.upClass, verticalDirection === "up")
        .toggleClass(options.downClass, verticalDirection === "down");
    }

    options.dropdown.toggle();
    parent
      .toggleClass(options.visibleClass, !visible)
    ...
    if (!visible) {
      ...
      options.dropdown.find(".dropdown-contents").each(function () {
        ...
        $this.toggleClass(
          "dropdown-nonscroll",
          this.scrollHeight === $this.innerHeight()
        );
      });
  ...
  phpbb.colorPalette = function (dir, width, height) {
    ...
    html +=
      '<table class="not-responsive colour-palette ' +
      tableClass +
      '" style="width: auto;">';
  ...
  phpbb.registerPalette = function (el) {
    var orientation =
        el.attr("data-color-palette") || el.attr("data-orientation"), // data-orientation kept for backwards compat.
      ...
      phpbb.recaptcha.form.on("submit", function (e) {
        ...
        if (!phpbb.recaptcha.ready) {
          ...
          if (phpbb.recaptcha.v3.length) {
            grecaptcha
              .execute(phpbb.recaptcha.v3.data("recaptcha-v3"), {
