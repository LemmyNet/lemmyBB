...
(function ($) {
  ...
  phpbb.addAjaxCallback("vote_poll", function (res) {
    if (typeof res.success !== "undefined") {
      ...
      if (!res.can_vote) {
        poll
          .find(".polls, .poll_max_votes, .poll_vote, .poll_option_select")
      ...
      poll.find("[data-poll-option-id]").each(function () {
        ...
        var percent = !res.total_votes
          ? 0
          : Math.round((res.vote_counts[optionId] / res.total_votes) * 100);
        var percentRel =
          mostVotes === 0
            ? 0
            : Math.round((res.vote_counts[optionId] / mostVotes) * 100);
        var altText;
